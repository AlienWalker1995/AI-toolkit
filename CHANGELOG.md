...
- **OpenClaw AGENTS / MCP tool names:** Verified that **`Tool not found`** for `gateway__duckduckgo__search`, `gateway__n8n__workflow_list`, etc. matches **invalid top-level ids** (only **`gateway__call`** / **`comfyui__call`** are registered), not a missing DuckDuckGo server when **`servers.txt`** lists **`duckduckgo`**. Long **`data/openclaw/workspace/AGENTS.md`** had misleading **`gateway__n8n_*`**-style bullets before OpenClaw’s **~20 k** bootstrap truncation and omitted the explicit wrong-name warning; local AGENTS wording updated; **`TROUBLESHOOTING.md`** documents truncation + **`gateway__n8n__workflow_list`**.
